...
public class BlockTutorialPortal extends BlockBreakable {
	...
	public BlockTutorialPortal() {
		...
		this.setRegistryName("tutorial_portal");
		this.setUnlocalizedName("blocktutorialportal");
	}
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		if(!entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn.isNonBoss() && entityIn instanceof EntityPlayerMP) {
			...
			} else if (player.dimension != TutorialDimensions.TUTORIAL.getId()) {
				player.timeUntilPortal = 10;
				player.mcServer.getPlayerList().transferPlayerToDimension(player, TutorialDimensions.TUTORIAL.getId(), new TutorialTeleporter(player.mcServer.getWorld(TutorialDimensions.TUTORIAL.getId())));
			} else {
				player.timeUntilPortal = 10;
				player.mcServer.getPlayerList().transferPlayerToDimension(player, 0, new TutorialTeleporter(player.mcServer.getWorld(0)));
			}
		}
	}
	...
	public static class Size {
		...
		protected int getDistanceUntilEdge(BlockPos pos, EnumFacing facing) {
			...
			for(i = 0; i < 22; ++i) {
				...
				if(!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock()) || this.world.getBlockState(blockpos.down()).getBlock() != TutorialBlocks.DUAL_FURNACE) break;
			}
			
			Block block = this.world.getBlockState(pos.offset(facing, i)).getBlock();
			return block == TutorialBlocks.DUAL_FURNACE ? i : 0;
		}
		...
		protected int calculatePortalHeight() {
			...
			for(this.height = 0; this.height < 21; ++this.height) {
				for(int i = 0; i < this.width; ++i) {
					...
					if(block == TutorialBlocks.PORTAL) ++this.portalBlockCount;
					
					if(i == 0) {
						block = this.world.getBlockState(pos.offset(leftDir)).getBlock();
						if(block != TutorialBlocks.DUAL_FURNACE) break label56;
			...
			for(int j = 0; j < this.width; ++j) {
				if(this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != TutorialBlocks.DUAL_FURNACE) {
					this.height = 0;
					break;
				}
			}
		...
		protected boolean isEmptyBlock(Block block) {
			return block.getDefaultState().getMaterial() == Material.AIR || block == Blocks.FIRE || block == TutorialBlocks.PORTAL;
		}
		...
		public void placePortalBlocks() {
			for(int i = 0; i < this.width; ++i) {
				...
				for(int j = 0; j < this.height; ++j) this.world.setBlockState(pos.up(j), TutorialBlocks.PORTAL.getDefaultState().withProperty(BlockTutorialPortal.AXIS, this.axis), 2);
			}
		}
	}
}
